...
def remove_main_guard(code: str) -> str:
    ...
    return re.sub(pattern, 'main()', code)
...
def load_test_cases(problem_id):
    test_dir = f"/ephemeral/peiyao-workspace/data/taskset/{problem_id}/testdata/"
    
    # Find all input/output test files
    input_files = sorted(glob.glob(f"{test_dir}/*.in"))
    output_files = sorted(glob.glob(f"{test_dir}/*.out"))
...
def run_test(
    ...
    in_outs = load_test_cases(problem_id)
    if debug:
